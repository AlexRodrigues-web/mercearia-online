...
{
    /**
     * Obtém **todos os produtos** para exibição na Home (Carrossel)
     *
     * @return array Lista de produtos
     */
    public function obterTodosOsProdutos(): array
    {
        try {
            // 🔍 Faz a busca dos produtos corretamente
            $query = "SELECT id, nome, preco, imagem FROM produtos ORDER BY id";
            
            // ✅ Usa o método correto da classe Model (projetarTodos)
            $produtos = parent::projetarTodos($query);

            return $produtos ?: [];
        } catch (PDOException $e) {
            error_log("Erro ao carregar produtos para a Home: " . $e->getMessage());
            return [];
        }
    }
     ...
     */
    public function obterDadosIniciais(): array
    {
        try {
